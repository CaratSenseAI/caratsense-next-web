...
const PREFIX_SPEED = 55; // ms per character
...
const HeroOrganicNetwork = ({ onSunClick, theme = 'dark', isLoaded = false }) => {
  ...
  // Type the hero prefix character-by-character once the page is loaded
  useEffect(() => {
    if (!isLoaded) return;
    let len = 0;
    const typeChar = () => {
    ...
    prefixTimerRef.current = setTimeout(typeChar, 400); // brief pause before starting
  ...
  // Scroll listener — skipped when a planet card is open
  useEffect(() => {
    const onScroll = () => {
      if (selectedIdRef.current) return; // planet card open: ignore scroll position changes
      ...
      setProgress(p);
      // CSS class drives the background transition — browser handles the smooth fade
  ...
  // Block scroll and auto-close planet card when user tries to scroll while one is open
  useEffect(() => {
    const onWheel = (e) => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Icon pop: staggered scale burst when icons fully converge
      ...
      particlesRef.current.forEach((pt, i) => {
        ...
        if (el) {
          // Burst scale: quick pop when dark reveal fires
          let burstScale = 1;
      ...
      // Update Hero Text Opacity (only fade once loaded so motion entrance isn't clobbered)
      if (heroTextRef.current) {
        ...
        // Canvas fades in as hero text fades out
    ...
    <div ref={wrapperRef} className="hon-wrapper" style={{ height: '250vh' }}>
      <div ref={bgContainerRef} className="hon-sticky" style={{ height: '100vh', position: 'sticky', top: 0, overflow: 'hidden' }}>
        ...
        <div ref={heroTextRef} className="hon-hero-text" style={{
          ...
          {/* Hero title — types in, then last word cycles */}
          <h1
          ...
          >
            {(() => {
              ...
              const p2 = "chaos into";
              const p2Start = p1.length + 1; // offset past the \n
              ...
              // Cursor visible while prefix is still being typed
